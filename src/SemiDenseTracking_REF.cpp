...
SemiDenseTracking::SemiDenseTracking()
{
   ...
   boost::filesystem::remove_all((ros::package::getPath("coke_slam")+"/src/map_and_poses").c_str());
   boost::filesystem::create_directory((ros::package::getPath("coke_slam")+"/src/map_and_poses").c_str());
   boost::filesystem::remove_all((ros::package::getPath("coke_slam")+"/src/evaluation").c_str());
   boost::filesystem::create_directory((ros::package::getPath("coke_slam")+"/src/evaluation").c_str());
   boost::filesystem::remove_all((ros::package::getPath("coke_slam")+"/src/results_depth_maps").c_str());
   boost::filesystem::create_directory((ros::package::getPath("coke_slam")+"/src/results_depth_maps").c_str());
...
void semidense_tracking(Imagenes *images,SemiDenseMapping *semidense_mapper,\
                        SemiDenseTracking *semidense_tracker,DenseMapping *dense_mapper,MapShared *Map,ros::Publisher *odom_pub,ros::Publisher *pub_poses,ros::Publisher *vis_pub,image_transport::Publisher *pub_image)
{
    if  (*semidense_tracker->cont_frames > semidense_tracker->last_cont_frames)//why?
    {
            ...
            semidense_tracker->last_cont_frames = *semidense_tracker->cont_frames;//OK.Swapped. But need to find where it is calculated/updated.
            ...
            if (semidense_tracker->image_n > semidense_tracker->last_frame_tracked)//Checks if a new image has come.
            {
             if (semidense_mapper->do_initialization_tracking < 0.5 )//This is hacky. Need to use mutexes
             {
                 semidense_tracker->image_prev = semidense_tracker->image_rgb.clone();//image_prev now has the the rgb_image that is yet to be tracked.
             }
             if (semidense_mapper->do_initialization_tracking > 0.5 )//WHY. How is this frame used?
             {

                 cv::Mat R_kf = images->Im[0]->R.clone();//defined in Superpixels.h;Im is of Imagen class. Has R n T component.
                 cv::Mat t_kf = images->Im[0]->t.clone();//B:first time it is cloned. Not sure when it is updated.
                 R_kf.convertTo(R_kf,CV_64FC1);
                 t_kf.convertTo(t_kf,CV_64FC1);

                 for (int i = 0;i<images->getNumberOfImages() ; i++)//clears the Im in Imagenes class.Must get updated in map
                 {
                    delete images->Im[i];
                    images->Im[i] = NULL;
                 }
                 ...
                 //Gets the Points that have been mapped. Commenting as of now.
                ...
                */
                ...
                //Join maps function is called based on some thing that happens in mapper. havent read join maps as of now.Commenting
                /*
                ...
                */
                ...
                //again uses points_map. Commenting
                /*for (int j=0; j<semidense_tracker->pyramid_levels ; j++)
                {
                   ...
                   double &fx,double &fy, double &cx, double &cy, int distortion, int reduction)
{
    ...
    image_n++;//increase the image_frame count

    cv::cvtColor(image_rgb,image_to_track,CV_RGB2GRAY);//convert it back to gray and save it in image_to_track

    image_to_track.convertTo(image_to_track, CV_64FC1);//convert it into 64bits floating point data.//GUESS:63 bits,double,gray image,1 channel
    image_gray = image_to_track.clone();//image_gray is has the processes image

    image_to_track /= (255*1.0);//why?
                    ...
                    int &pyramid_levels,vector<int> &reduction_pyramid, vector<cv::Mat> &image_keyframe_pyramid, int reduction)
{
   ...
   image_rgb.copyTo(image_keyframe);//the image_keyframe now has the image_rgb.NOTE:not the image_gray or image_to_track
   cv::Mat image_p = image_rgb.clone();//B:Whats the difference between copyTo and clone?

   R.copyTo(R1);
   t.copyTo(t1);

   cv::cvtColor(image_p,image_p,CV_RGB2GRAY);//A:again the same thing. Hasnt this been done for image_gray and image_to_track?
   ...
   for (int j=0; j<pyramid_levels ; j++)
   {
         //A:Interesting how the pyramid is calculated. There is already a cv:function to calculate the pyramid
         ...
         //Commenting cause no idea what happens here.Also cause points_map have not been initialized.
...
//I have no idea how this function works. Disabling as of now.
